...
sub processOrganism {
  ...
  my $includesTRNAs = ($orgHash->{annotationIncludesTRNAs});
...
#  &addClonedInsertEnds($result, $datasetProps, $webservicesDir, $nameForFileNames, $projectName, $applicationType, $buildNumber);
  ...
  &addTRNA($datasetProps, $result, $nameForFileNames, $projectName, $buildNumber, $applicationType, $includesTRNAs);
...
sub addTRNA {
  my ($datasetProps, $result, $nameForFileNames, $projectName, $buildNumber, $applicationType, $includesTRNAs) = @_;
     ...
     my $relativePathToGffFile;

     if ($includesTRNAs eq 'true') {
     $relativePathToGffFile = "${nameForFileNames}/genomeAndProteome/gff/annotated_transcripts.gff.gz";
     } else {
     $relativePathToGffFile = "${nameForFileNames}/genomeAndProteome/gff/tRNAScan.gff.gz";
     }
...
sub addOrfs {
  my ($result, $datasetProps, $webservicesDir, $nameForFileNames, $projectName, $applicationType, $buildNumber) = @_;
     # TODO: need to switch from annotated_transcripts.gff.gz to new tRNA scan gff generated by workflow
     my $relativePathToGffFile = "${nameForFileNames}/genomeAndProteome/gff/Orf50.gff.gz";

     my $track = ApiCommonModel::Model::JBrowseTrackConfig::OrfTrackConfig->new({
                                                                                   project_name => $projectName,
                                                                                   build_number => $buildNumber,
                                                                                   relative_path_to_file => $relativePathToGffFile,
                                                                                   application_type => $applicationType
