...
/**
 * This is a modified copy of ApiExample.java -- Java example using the Google Fusion Tables API to query,
 * insert, update, and delete. Uses the Google GDATA core library.
 *
...
public class FusionTable {
  ...
  /**
   * Google Fusion Tables API URL stem. All requests to the Google Fusion Tables server begin with this URL.
   *
  ...
  /**
   * CSV values are terminated by comma or end-of-line and consist either of plain text without commas or
   * quotes, or a quoted expression, where inner quotes are escaped by doubling.
   */
   ...
   *
   * This code uses the GoogleService class from the Google GData APIs Client Library.
   */
  private GoogleService service;

  /**
   * Two versions of FusionTable() are provided: one that accepts a Google user account ID and password for
   * authentication, and one that accepts an existing auth token.
   */

  /**
   * Authenticates the given account for {@code fusiontables} service using a given email ID and password.
   *
   * @param email
   *          Google account email. (For more information, see http://www.google.com/support/accounts.)
   * @param password
   *          Password for the given Google account.
   *
   *          This code instantiates the GoogleService class from the Google GData APIs Client Library,
   *          passing in Google Fusion Tables API-specific parameters. It then goes back to the Google GData
   *          APIs Client Library for the setUserCredentials() method.
   */
  public FusionTable(String email, String password) throws AuthenticationException {
  ...
  /**
   * Authenticates for {@code fusiontables} service using the auth token. The auth token can be retrieved for
   * an authenticated user by invoking service.getAuthToken() on the email and password. The auth token can be
   ...
   *           if unable to authenticate with passed token
   *
   *           This code instantiates the GoogleService class from the Google Data APIs Client Library,
   *           passing in Google Fusion Tables API-specific parameters. It then goes back to the Google Data
   *           APIs Client Library for the setUserToken() method.
   */
  ...
  /**
   * Fetches the results for a select query. Prints them to standard output, surrounding every field with
   * (@code |}.
   *
   * This code uses the GDataRequest class and getRequestFactory() method from the Google Data APIs Client
   * Library. The Google Fusion Tables API-specific part is in the construction of the service URL. A Google
   * Fusion Tables API SELECT statement will be passed in to this method in the selectQuery parameter.
   */
  public void loadTuningTableFromFusionTable(String datasetId, Connection dbc, String tuningTable,
      String suffix) throws IOException, ServiceException, SQLException {
   ...
   * Fusion Tables API SELECT statement will be passed in to this method in the selectQuery parameter.
   */
  public void createTuningTable(String datasetId, Connection dbc, String tuningTable, String suffix,
      String expectedColumns) throws IOException, ServiceException, SQLException, Exception {
    ...
    try (Scanner scanner = new Scanner(request.getResponseStream(), "UTF-8")) {
      boolean processingHeader = true;
      int columnNumber = 0;
      int columnCount = 0;
      int MAXCOLUMNS = 30;
        ...
        if (processingHeader) {
          ...
          if (expectedColumns.length() > 0) {
            if (!expectedColumnSet.contains(decoded))
              throw new Exception("column \"" + decoded + "\" not contained in expected-column list \"" +
                  expectedColumns + "\" from URL \"" + urlString + "\"");
          }
          ...
          if (!match.group(4).equals(",")) { // last column; process row
            ...
            if (expectedColumnSet.size() != columnCount) {
              throw new Exception("expected " + expectedColumnSet.size() + " columns; received " +
                  columnCount + " from URL \"" + urlString);
            }
          }
        }
        else {
