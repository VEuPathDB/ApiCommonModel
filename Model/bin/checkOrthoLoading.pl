...
my ($help, $databaseInstance, $legacyDatabaseInstance, $dataDir, $logFile,$newLogFile);

&GetOptions('help|h' => \$help,
            'database_instance=s' => \$databaseInstance,
            'workflow_data_dir=s' => \$dataDir,
            'log_file=s' => \$logFile
            'new_log_file=s' => \$newLogFile
            );


my $dbh = &connectToDb($databaseInstance);
my $legacyDbh = &connectToDb($legacyDatabaseInstance);
