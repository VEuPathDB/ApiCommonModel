...
my $sharedParamsXml = $ENV{PROJECT_HOME} . "/ApiCommonModel/Model/lib/wdk/apiCommonModel/questions/params/sharedParams.xml";
...
my $sh = $dbh->prepare("select distinct project_id from webready.GenomicSeqAttributes_p");
$sh->execute();
